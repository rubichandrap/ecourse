<div class="row">
    <div class="col-9">
        <h2>{{ $session->title }} [{{ $session->status == '0' ? 'closed' : 'active' }}]</h2>
    </div>
    <div class="col-3 text-right">
        @include('template.showSessionElipsisMenu')
    </div>
</div>
<p class="mb-0">{{ strip_tags($session->desc) }}</p>
<hr class="mt-3 mb-4">
@if($session->status == '1')
<form id="form-postings" class="mb-5" method="POST" action="{{route('posting.store', [$kelas->id, $session->session_of])}}" enctype="multipart/form-data">
    @csrf
    <div class="row">
        <div class="col-9">
            <textarea class="form-control" type="text" name="content" rows="3" maxlength="255" placeholder="Katakan sesuatu..." style="resize: none;"></textarea>
        </div>
...
@foreach($postings as $key)
<div class="card mb-3 postings" data-target="{{ $key->user->nbm_nim }}">
    <div class="card-header post-header border-0 pb-0 pt-3">
        <div class="row">
            <div class="col-9">
                <div class="row">
                    <div class="col-12">
            ...
            <div class="col-3 text-right">
                ...
                <form method="POST" action="{{ route('posting.destroy', $key->id) }}">
                    @csrf
    ...
    <div class="comment-wrapper bg-secondary" style="display: none;">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="d-inline-block">
                                            <p class="mb-0 text-muted data-username" style="font-size:.7rem;">{{ '@'.$comment->user->username }}</p>
        ...
        <div class="pl-5 py-3">
            @if($session->status == '1')
            <form id="form-comments" method="POST" action="{{route('comment.store', $key->id)}}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-9">
